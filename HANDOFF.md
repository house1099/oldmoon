...
5. **2026-03-31 — 獎池權重輸入與私訊日期**：**`prizes-client.tsx`** 權改用 **`weightStr`** 控制輸入（可清空再輸入）；**`saveAllWeights`／新增獎項** 送出前驗證 **≥1**；機率加總用 **`resolvedPrizeItemWeight`**（空或非法暫回伺服器權重）。**`ChatModal.tsx`** 依 **台北曆日** 插入 **今天／昨天／月日（週）** 分隔列。詳見 **`HANDOFF_HISTORY.md`**（**「獎池權重輸入與私訊日期（2026-03-31）」**）。
