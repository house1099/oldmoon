...
| 使用者詳情 Modal | **`UserDetailModal.tsx`**：**IG** 僅在 **`instagram_handle` 有值** 且（**`ig_public === true`** 或 **血盟 `accepted`**）時顯示；**「在 Instagram 開啟」**（**`https://www.instagram.com/{handle}/`**，**`instagram.ts`** strip **`@`**）；基本資訊內 **今日心情**（**`mood` + `mood_at` 24h 內**）；關閉 **`ChatModal`** 時 **`mutate`** **`conversations`／`unreadChatConversations`**；**master** 可開啟 **`LeaderToolsSheet`**（信譽分與 **<30** 警告、IG 強制顯示、EXP 發放、**邀請碼改走信件 `invitation_code`**、放逐）；層級 **`z-[600]`／`z-[610]`** |
...
- **前台領袖快捷面板**：**`UserDetailModal.tsx`** 當 **`myProfile.role === 'master'`** 時 DialogFooter 底部顯示 **「⚡ 領袖工具」** 按鈕；點擊開啟 **`LeaderToolsSheet.tsx`**（固定右側滑出 **`w-80`**，**`z-[620]`／`z-[630]`**）；載入時 **`getMemberProfileByIdAction`** 取完整資料（含 IG）。
