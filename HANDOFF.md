...
1. **2026-03-31 — 探索 `UserCard` 稱號約 1.3× 與排版**：**`TitleBadgeRow`** 新增 **`size="lg"`**（胸章 **1.3rem**、字 **13px**、**`tracking-tight`／`font-medium`**、寬螢 **CSS truncate**）；**`UserCard`** 採 **`lg`**、頂緣 **`-translate-y-[42%]`**、內容 **上內距 `pt-6`**、膠囊 **髮線邊＋微漸層＋內高光＋淡 ring**。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號放大與排版（Apple-like，2026-03-31）」**）。
2. **2026-03-31 — 探索列表稱號補齊與卡片頂標**：**`ExploreClient`** 村莊 **`useSWR`** 改 **`revalidateOnMount`／`revalidateIfStale`／`keepPreviousData`**；**`village.service`** **`unstable_cache`** 鍵 **`village-v7`**；**`UserCard`** 稱號改 **absolute** 貼 **卡片上緣中央**（避免 flex 擠壓）。詳見 **`HANDOFF_HISTORY.md`**（**「探索列表稱號遺漏：村莊 SWR 重拉、`village-v7`、卡片頂緣標籤」**）。
3. **2026-03-31 — 稱號胸章全站＋裝備同步＋後台**：L2 **`findEquippedTitlesByUserIds`**、**`findEquippedRewardLabels.equippedTitleImageUrl`**；L3 村莊／市集／私訊／酒館／血盟 **`equippedTitle`／`equippedTitleImageUrl`**；L5 **`TitleBadgeRow`**（**`UserCard`／`UserDetailModal`／`guild-profile-home`／公會聊天與血盟列表／`ChatModal`／`TavernModal`**）、**`ExploreClient`** 預載胸章圖；**`FloatingToolbar`** 裝卸 **`title`／`avatar_frame`／`card_frame`** 後 **`guild-rewards-invalidate`** → 首頁 **`loadRewards`**；後台 **`shop-admin`** **`title` optgroup**、**`prizes`** **`title`** 與 **`LocalFrameImagePicker`**（**`items`**）；**`createPrizeItemAction`／`updatePrizeItemAction`** 允許 **`title`** 寫入 **`image_url`／`effect_key`**。詳見 **`HANDOFF_HISTORY.md`**（**「稱號胸章全站顯示與後台（2026-03-31）」**）。
4. **2026-03-31 — 後台獎項：框架類連動商城選圖**：**`prizes-client.tsx`** 於 **`avatar_frame`／`card_frame`** 使用 **`LocalFrameImagePicker`**（與 **`shop-admin-client`** 相同 **`getShopLocalImageOptionsAction`** **optgroup**）；**「從商城商品帶入」** 以 **`getShopItemsAdminAction`** 寫入 **`label`／`effect_key`／`image_url`**（卡框 **metadata** 圖層不寫入 **`prize_items`**）。詳見 **`HANDOFF_HISTORY.md`**（**「後台獎項：框架類連動商城選圖（2026-03-31）」**）。
5. **2026-03-31 — PWA 首頁安裝引導**：**`usePwaInstall`** 攔截 **`beforeinstallprompt`**；**`PwaInstallOverlay`** 底部抽屜（深紫邊、**standalone／fullscreen／iOS `navigator.standalone`** 即隱藏）；**`pwa_prompt_dismissed`** **3 日**冷卻；**session** 內須先 **簽到（`checkinDone`）** 或 **進公會頁** 才於首頁顯示。詳見 **`HANDOFF_HISTORY.md`**（**「PWA 首頁安裝引導（2026-03-31）」**）。
