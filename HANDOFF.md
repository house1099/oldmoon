...
- 月老釣魚（不公開顯示於卡片）：`relationship_status`、`birth_year`、`matchmaker_age_range`、`matchmaker_region_pref`（JSON 字串）
...
- 型別：`src/types/database.types.ts`；SWR：`src/lib/swr/keys.ts`；等級：`src/lib/constants/levels.ts`；標籤：`src/lib/constants/tags.ts`；月老地區：`src/lib/utils/matchmaker-region.ts`

**舊路由**：`/village`、`/market` → `/explore`；`/alliances`、`/inbox` → `/guild`。

## 🗄️ 資料庫表清單

- `users` — 會員主檔（暱稱、等級、`total_exp`、自白、標籤、幣、狀態、月老釣魚偏好欄位等）
...
1. **2026-04-01 — 釣魚系統地基（月老偏好）**：遷移 **`20260401200000_users_fishing_fields.sql`**；雲端 **Supabase MCP `execute_sql`**；**`users`** 新增 **`relationship_status`／`birth_year`／`matchmaker_age_range`／`matchmaker_region_pref`**；**`database.types.ts`**；註冊 **`profile-form.tsx`** Step2 出生年＋感情狀態；**`completeAdventurerProfile`**／**`updateMyProfile`** 驗證與寫入；**`guild-profile-home.tsx`** 帳號設定「🎣 月老釣魚偏好」＋地區 **Dialog**；**`matchmaker-region.ts`**。**`npx tsc --noEmit`**、**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「釣魚系統地基（2026-04-01）」**）。
2. **2026-04-01 — 自由市場橘點已讀＋後台按鈕對比**：**`FloatingToolbar`** 售出橘點改為僅在 **關閉 `MarketSheet` 後**仍 **24h 內新成交**時顯示（**`lastMarketSheetClosedAt`＋`sessionStorage` `ft_market_sheet_closed_at`**，對齊酒館「關閉即已讀」）；快捷列文字膠囊改 **白字／較不透明底**（含 **廣播管理**）。**`Button`** 新增 **`outlineLight`**（白底深字，不受 **`dark` 根**影響）；**`market-admin-client`／`shop-admin-client`／`prizes-client`** 之 **`outline`** 改 **`outlineLight`** 修正分頁等灰字看不清。**`npx tsc --noEmit`**、**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「自由市場橘點已讀與後台按鈕對比（2026-04-01）」**）。
3. **2026-04-01 — 玩家自由市場 RLS／封號下架／後台市場**：遷移 **`20260401100000_market_listings_rls.sql`**（**`market_listings` RLS**、**authenticated** 讀 **active**／本人全狀態）、**`20260401100500_market_enabled_setting.sql`**（**`market_enabled`**）；雲端 **Supabase MCP `execute_sql`** 同步；**`cancelAllActiveListingsByUser`**＋**`banUserAction`**；**`market-listing.repository.ts`** **`getMarketStats`／`findAllListingsForAdmin`／`findSoldListingsForAdmin`／`adminCancelListing`／`findSuspiciousListings`**、**`buyer`** embed；**`market-listing.action.ts`** **`market_enabled`** 攔截 **create/buy**（**`market_disabled`**）、後台 actions、**`updateMarketSettingsAction`**；**`admin.action.ts`** **`export requireRole`**；**`/admin/market`**（**`market-admin-client.tsx`** 五 Tab）；**`admin-shell.tsx`** **市場管理**、**`middleware.ts`** 版主 **`/admin/market`**；**`MarketSheet`／`FloatingToolbar`** 關閉拍賣場提示。**`npx tsc --noEmit`**、**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「玩家自由市場 RLS／封號下架／後台市場（2026-04-01）」**）。
4. **2026-03-31 — 自由市場 UI 補齊**：**`market-listing.repository.ts`** **`findRecentSoldListings`**／**`RecentSoldItem`**；**`market-listing.action.ts`** **`getRecentSoldListingsAction`**（**`unstable_cache` 60s**、**`tags: ['market_sold']`**）、**`buyListingAction`** 成功 **`revalidateTag('market_sold')`**；**`SWR_KEYS.marketRecentSold`**；**`globals.css`** **`market-ticker`** 跑馬燈；**`MarketSheet.tsx`** 標題列（關閉／**＋ 上架**）、Tab **拍賣市集**、行情列、列表 **40×40** 預覽（**`next/image`／特效／emoji**）、**`by @暱稱`**、上架 **Dialog**（**`getMyRewardsAction`**＋**`createListingAction`**）；**`FloatingToolbar`** 子鈕 **`items-end`／固定列寬**、**「自由市場」** 文案。**`npx tsc --noEmit`**、**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「自由市場 UI 補齊（2026-03-31）」**）。
5. **2026-03-31 — 玩家自由市場 L2／L3／L5**：**`market-listing.repository.ts`**（admin **`createListing`／`findActiveListings`／`findMyListings`／RPC／`expireMyStaleListings`／`countActiveListingsBySeller`** 等）；**`market-listing.action.ts`**（**`createListingAction`** 含 **`unequipReward`**、上限與 **`allow_player_trade`**、**`buyListingAction`** 賣家 **`notifyUserMailboxSilent`＋`sendPushToUser`**）；**`MarketSheet.tsx`** 大廳篩選購買／我的上架下架；**`FloatingToolbar`** 第四子鈕 **Store**（**`SWR_KEYS.myMarketListings`**、24h 售出橘點）、背包長按 **「上架至市集」**；**`rewards.repository`** **`allow_player_trade`**。**`npx tsc --noEmit`**、**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「玩家自由市場 L2／L3／L5（2026-03-31）」**）。
