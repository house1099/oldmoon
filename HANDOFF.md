...
- 獎勵／廣播／改名／贈禮：`rewards.action.ts`；`rewards.repository.ts`（**`findEquippedTitlesByUserIds`**、**`findEquippedRewardLabels`** 之 **`equippedTitleImageUrl`**）；`gift.action.ts`（暱稱搜尋贈送玩家）；`system-settings`
...
- **後台獎項／獎池**：**`(admin)/admin/prizes/prizes-client.tsx`**（**`avatar_frame`／`card_frame`／`title`**：**`LocalFrameImagePicker`** 含 **`public/items` optgroup**；商城帶入）；**`components/admin/local-frame-image-picker.tsx`**；**`(admin)/admin/shop/shop-admin-client.tsx`**（**`title`** 本機圖 **optgroup**、胸章小預覽）
...
- 卡片／Modal：`UserCard.tsx`；**`title-badge-row.tsx`**（稱號胸章＋膠囊；**`sm`／`md`／`lg`／`xl`**；探索 **`UserCard`** 稱號在**底列**興趣／技能右側 **`xl`**）；`public/items/title-luffy.png`（稱號胸章範例 **`LUFFY`**，見 **`public/items/README.md`**）；`CardDecorationWrapper.tsx`；`card-decoration.ts`（`CardDecorationConfig`／metadata 解析）；`modals/UserDetailModal.tsx`；`LevelBadge`／`LevelCardEffect`；`ShopCardFrameOverlay.tsx`；卡框比例 **`CARD_FRAME_OVERLAY_PERCENT`**（`shop-card-frame-preview.ts`，與頭像框 **`MASTER_AVATAR_FRAME_OVERLAY_PERCENT`** 分離）
...
1. **2026-03-31 — 探索 `UserCard` 稱號底列＋LUFFY 胸章資產**：稱號改置 **興趣／技能列右側**（**`items-center`** 垂直對齊標籤）；**`TitleBadgeRow`** 新增 **`size="xl"`**；**`public/items/title-luffy.png`**（白底原圖 **`source-luffy-72.png`** 邊緣 flood-fill 去背＋**1.3×**）、**`scripts/process-title-luffy-png.py`**；**`public/items/README.md`** 後台填 **`/items/title-luffy.png`**、名稱 **LUFFY**。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號底列與 LUFFY 胸章（2026-03-31）」**）。
2. **2026-03-31 — 探索 `UserCard` 稱號約 1.3× 與排版**：**`TitleBadgeRow`** 新增 **`size="lg"`**（胸章 **1.3rem**、字 **13px** 等）；**`UserCard`** 曾用頂緣稱號（後已由底列取代）。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號放大與排版（Apple-like，2026-03-31）」**）。
3. **2026-03-31 — 探索列表稱號補齊與卡片頂標**：**`ExploreClient`** 村莊 **`useSWR`** 改 **`revalidateOnMount`／`revalidateIfStale`／`keepPreviousData`**；**`village.service`** **`unstable_cache`** 鍵 **`village-v7`**；**`UserCard`** 稱號曾改 **absolute** 貼 **卡片上緣中央**（避免 flex 擠壓）。詳見 **`HANDOFF_HISTORY.md`**（**「探索列表稱號遺漏：村莊 SWR 重拉、`village-v7`、卡片頂緣標籤」**）。
4. **2026-03-31 — 稱號胸章全站＋裝備同步＋後台**：L2 **`findEquippedTitlesByUserIds`**、**`findEquippedRewardLabels.equippedTitleImageUrl`**；L3 村莊／市集／私訊／酒館／血盟 **`equippedTitle`／`equippedTitleImageUrl`**；L5 **`TitleBadgeRow`**（**`UserCard`／`UserDetailModal`／`guild-profile-home`／公會聊天與血盟列表／`ChatModal`／`TavernModal`**）、**`ExploreClient`** 預載胸章圖；**`FloatingToolbar`** 裝卸 **`title`／`avatar_frame`／`card_frame`** 後 **`guild-rewards-invalidate`** → 首頁 **`loadRewards`**；後台 **`shop-admin`** **`title` optgroup**、**`prizes`** **`title`** 與 **`LocalFrameImagePicker`**（**`items`**）；**`createPrizeItemAction`／`updatePrizeItemAction`** 允許 **`title`** 寫入 **`image_url`／`effect_key`**。詳見 **`HANDOFF_HISTORY.md`**（**「稱號胸章全站顯示與後台（2026-03-31）」**）。
5. **2026-03-31 — 後台獎項：框架類連動商城選圖**：**`prizes-client.tsx`** 於 **`avatar_frame`／`card_frame`** 使用 **`LocalFrameImagePicker`**（與 **`shop-admin-client`** 相同 **`getShopLocalImageOptionsAction`** **optgroup**）；**「從商城商品帶入」** 以 **`getShopItemsAdminAction`** 寫入 **`label`／`effect_key`／`image_url`**（卡框 **metadata** 圖層不寫入 **`prize_items`**）。詳見 **`HANDOFF_HISTORY.md`**（**「後台獎項：框架類連動商城選圖（2026-03-31）」**）。
