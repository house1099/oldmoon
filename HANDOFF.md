...
1. **2026-03-31 — 獎池權重輸入與私訊日期**：**`prizes-client.tsx`** 權改用 **`weightStr`** 控制輸入（可清空再輸入）；**`saveAllWeights`／新增獎項** 送出前驗證 **≥1**；機率加總用 **`resolvedPrizeItemWeight`**（空或非法暫回伺服器權重）。**`ChatModal.tsx`** 依 **台北曆日** 插入 **今天／昨天／月日（週）** 分隔列。詳見 **`HANDOFF_HISTORY.md`**（**「獎池權重輸入與私訊日期（2026-03-31）」**）。
2. **2026-03-31 — 贈禮單則通知（一動作一信件／Push）**：**`gift.action.ts`** 匯出 **`formatGiftBatchMailboxMessage`**，**`confirmGiftsToUserBatchAction`** 沿用；**`FloatingToolbar`** 背包贈一般玩家改 **`giftPlayerRewardIds`＋`confirmGiftsToUserBatchAction`**（**`stackMenuQty`** 多件同次送出僅一則通知）；**`rewards.action.ts`** **`giftUserRewardsToAlliancePartnerBatchAction`** 成功後 **`notifyUserMailboxSilent`** 一則摘要（同文案 helper）。商城 **`confirmGiftsToUserBatchAction`** 行為不變。詳見 **`HANDOFF_HISTORY.md`**（**「贈禮單則通知與共用信件文案（2026-03-31）」**）。
3. **2026-03-31 — 酒館 `@` 提及改輸入觸發＋篩選**：**`tavern-mentions.ts`** **`getTavernInlineMentionState`**；**`TavernModal`** 游標同步、直向名單、**`includes`** 篩選、**`↑`／`↓`／`Enter`／`Esc`**、左側 **`@`** 改 **插入 `@`**；與貼圖列互斥。詳見 **`HANDOFF_HISTORY.md`**（**「酒館 @ 提及輸入觸發與篩選（2026-03-31）」**）。
4. **2026-03-31 — 探索 `UserCard` 底列稱號 `card`（約 sm×1.1）**：**`TitleBadgeRow`** 新增 **`size="card"`**（胸章 **1.1rem**、字 **11px**、較窄 **`max-w`**）；**`UserCard`** 底列 **`xl` → `card`**，避免擠壓興趣／技能標籤。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 底列稱號縮小（`card` ≈ sm×1.1，2026-03-31）」**）。
5. **2026-03-31 — 探索 `UserCard` 稱號底列＋LUFFY 胸章資產**：稱號改置 **興趣／技能列右側**（**`items-center`** 垂直對齊）；**`public/items/title-luffy.png`**（**`source-luffy-72.png`** flood-fill 去背＋**1.3×**）、**`scripts/process-title-luffy-png.py`**；**`public/items/README.md`** 後台 **`/items/title-luffy.png`**、名稱 **LUFFY**。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號底列與 LUFFY 胸章（2026-03-31）」**）。
