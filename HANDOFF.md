...
- **關鍵檔案**：**`src/lib/utils/fishing-shop-metadata.ts`**、**`src/app/(admin)/admin/shop/shop-admin-client.tsx`**、**`src/components/matchmaking/fishing-panel.tsx`**、**`src/app/(app)/matchmaking/page.tsx`**、**`src/services/fishing.action.ts`**、**`src/lib/repositories/server/fishing-cast.repository.ts`**。
