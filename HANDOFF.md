...
- **Layer 2（資料）**：`user.repository.ts`（含 **`updateLastCheckinAt`**）、`exp.repository.ts` 支援 **`total_exp`**（SSOT）；**`streak.repository.ts`**（**`login_streaks`**）、**`prize.repository.ts`**（**`prize_pools`／`prize_items`／`prize_logs`／`user_rewards`**，含獎池／獎項 CRUD 與 log 計數）、**`rewards.repository.ts`**（**`user_rewards` 讀寫裝備**、**`broadcasts`**）；**`chat.repository.ts`**（**`conversations`**／**`chat_messages`**／**`blocks`**／**`reports`**，admin，供私訊／封鎖／檢舉接線）。
- **Layer 3（業務）**：`daily-checkin.action.ts` 之 **`claimDailyCheckin`**（**`login_streaks`** 連簽、第 7 天 **`prize-engine.drawFromPool('loot_box')`**）以 **`users.last_checkin_at`** 為簽到 **24h 滾動冷卻** SSOT；**`exp_logs.unique_key`** 為 **`daily_checkin:{userId}:{timestamp}`**；**`prize-engine.ts`** 通用抽獎 helper；**`rewards.action.ts`**（**我的獎勵**、**廣播券**、**`getActiveBroadcastsAction`** 快取）；**`chat.action.ts`**（開啟對話／訊息／列表／封鎖／檢舉）、**`notification.action.ts`**（**`getMyNotificationsAction`**（**直接** **`loadNotificationsForUser`**，**不**包 **`unstable_cache`**；批量載入發送者）／**`insertMailboxNotificationAction`／`notifyUserMailboxSilent`**／**`markNotificationReadAction`**／**`markAllNotificationsReadAction`**／清除／未讀數）。
...
- **Layer 3**：**`rewards.action.ts`** — **`getMyRewardsAction`**（分組 **titles／avatarFrames／broadcasts** + **`broadcastUnusedCount`**）、**`equipRewardAction`／`unequipRewardAction`**（**`revalidateTag(profileCacheTag)`**）、**`useBroadcastAction`**（訊息 **1〜30** 字、**`revalidateTag('broadcasts')`**）、**`getActiveBroadcastsAction`**（**`unstable_cache` 60s** **`tags: ['broadcasts']`**）。**`admin.action`**：**`createPrizePoolAction`／`deletePrizePoolAction`／`createPrizeItemAction`／`deletePrizeItemAction`**，**`updatePrizeItemAction`** 含 **reward_type** 與 **coins/exp** 驗證；**`getPrizePoolsAction`／`getPrizeItemsAction`** 附 **`hasPrizeLogs`**。**`getMemberProfileByIdAction`** 回傳 **`equippedTitle`／`equippedFrame`**（僅 Modal 路徑查 **user_rewards**，列表不 JOIN）。
- **Layer 5 首頁**：**`page.tsx`** **`await getActiveBroadcastsAction()`** → **`GuildProfileHome`** **贊助橫幅與公告之間** — **廣播橫幅**（**amber** 樣式、**4s** 輪播、**`.broadcast-slide-in`**）；**「我的狀態」** 手風琴 **🎁 我的獎勵**（稱號／頭像框裝備、**星辰之框** 首頁頭像 **ring** 發光、廣播券 **Dialog** 預覽＋送出）。**`UserDetailModal`** 開啟時 **`getMemberProfileByIdAction`** 補齊裝備顯示；**`UserCard`** 可選 **`equippedTitle`** 膠囊（列表資料未帶則不顯示）。
...
- **`invitation_codes`**：**`id`**（uuid PK）、**`code`**（text UNIQUE，8碼英數大寫）、**`created_by`**（uuid → `users`）、**`used_by`**／**`used_at`**（相容欄位：首筆核銷時寫入）、**`max_uses`**（int，預設 **1**）、**`use_count`**（int，預設 **0**）、**`expires_at`**、**`is_revoked`**（boolean；額滿核銷時 RPC 可一併設為 true）、**`note`**、**`created_at`**。**`invitation_code_uses`**：**`id`**、**`code_id`** → **`invitation_codes`**、**`used_by`** → **`users`**、**`used_at`**（每筆核銷一列，完整追溯）。
- **邀請制規則（強制）**：註冊 Step1 **必須**有效邀請碼；**`signUp`** 將碼寫入 **`user_metadata.invite_code`**；**`completeAdventurerProfile`** 成功後呼叫 **`claimInvitationCodeAction`**（RPC **`claim_invitation_code`**，`FOR UPDATE` 原子核銷）→ 成功則 **`users.invited_by`** = 該張碼的 **`invitation_codes.created_by`**（推薦鏈）。
- **Layer 2**：**`invitation.repository.ts`** — **`findInvitationRowByCode`**（撞碼）、**`findInvitationByCode`**（僅「仍可使用」列）、**`findInvitationUsesByCodeId`**、**`claimInvitationCode`**（RPC）、**`findInvitationTree`**（附 **`registration_invite_code`**／**`registration_invite_meta`** 供樹狀圖）。**Layer 3 前台**：**`invitation.action.ts`** — **`validateInvitationCodeAction`**、**`claimInvitationCodeAction`**。**後台**：**`admin.action.ts`** — **`getInvitationCodeUsesAction`** 等（已移除 **`validateInviteCodeAction`**／**`claimInviteCodeAfterRegisterAction`**）。
...
- **`tavern_messages`**／**`tavern_bans`**：酒館廣場公開聊天與禁言。Layer 2 **`tavern.repository.ts`**（admin：**`findTavernMessages`**、**`insertTavernMessage`**、**`isTavernBanned`**、**`insertTavernBan`**／**`deleteTavernBan`**、**`findAllTavernBans`**、**`deleteTavernMessage`**）；Layer 3 **`tavern.action.ts`**（列表／發送／可操作禁言解除／刪訊／**`getTavernBansAction`**）；**Realtime** 需將 **`tavern_messages`** 加入 **`supabase_realtime`** publication（遷移 **`20260326120000_tavern_messages_and_bans.sql`**）；RLS：**`authenticated` SELECT** 訊息表以利即時訂閱，寫入僅後端 **service role**。
...
| 讀取他人 profile（Modal） | **`src/services/profile.action.ts`** — **`getMyProfileAction`**、**`getMemberProfileByIdAction`**（回傳 **`MemberProfileView`**：**`equippedTitle`／`equippedFrame`**） |
| 前台獎勵／廣播 | **`src/services/rewards.action.ts`**；**`src/lib/repositories/server/rewards.repository.ts`** |
...
| 管理員後台 | **src/app/(admin)/layout.tsx**（獨立 layout、**`AdminShell`** sidebar；根層 **`text-gray-900`** + **`[color-scheme:light]`**）；**/admin**（儀表板 **`page.tsx`** 為 client：統計卡 **`router.push`** 至 **`/admin/users?filter=…`**／**`/admin/reports?filter=pending`** 等）、**/admin/users**（**`searchParams.filter`**：`today`／`pending`／`active`／`ig_pending`；見檔末變更紀錄）、**/admin/invitations**、**/admin/exp**、**/admin/prizes**（獎池／抽獎紀錄，**master only**）、**/admin/publish**、**/admin/reports**、**/admin/roles**（master only）、**/admin/settings**（Wave 2） |
...
| 首頁個人頁 UI | **`page.tsx`**（async RSC）**`await getMessageLimitsAction()`** → **`home-page-client.tsx`**（**`'use client'`**、**`useMyProfile`** SWR + **`HomePageSkeleton`**、背景 **`HomeParticlesBackground`** 等）→ **`GuildProfileHome`**（**`moodMax`** 對應 **`mood_max_length`**）。**公告上方**：**banner 輪播**；**公告區塊**（**滿版垂直堆疊**、**`line-clamp-2`＋「⋯ 展開」**、整卡 **Dialog**）；**今日心情下方**：**card 贊助橫滑**；**每日簽到成功**為 **遊戲感 Dialog** |
...
| 酒館廣場（全頁） | **`TavernMarquee`**（**`src/components/tavern/TavernMarquee.tsx`**）：頂欄跑馬燈最新 5 則、**`useTavern`**；**`(app)/layout`** 傳 **`messageMaxLength`**（**`getMessageLimitsAction().tavernMax`**）給 **`TavernFab` → `TavernModal`**（**`maxLength`**）；全螢幕聊天、貼圖、**master/moderator** 長按刪除與禁言；**`globals.css`** **`.animate-marquee`**。 |
...
| 後台酒館禁言 | **`/admin/users`** **`users-client.tsx`**（viewerRole 為 **master/moderator**，且需符合操作權限規則）：**`AlertDialog`** → **`banTavernUserAction`**／**`unbanTavernUserAction`**；**`getUserDetailAction`** 回傳含 **`tavern_banned`**。 |
