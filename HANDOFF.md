...
| 首頁個人頁 UI | `src/app/(app)/page.tsx`（**`'use client'`**、**`useMyProfile`** SWR + **`HomePageSkeleton`**、背景 **`HomeParticlesBackground`**：**`fetch('/particles.json')`**（**`public/particles.json`**），失敗則退回 **`src/config/home-particles.json`**；**`fixed inset-0 z-0`** 全螢幕、內容 **`z-10`**） → `guild-profile-home.tsx`（**公告上方**：**banner 輪播**；**公告區塊**（**滿版垂直堆疊**、**`line-clamp-2`＋「⋯ 展開」**、整卡 **Dialog**）；**今日心情下方**：**card 贊助橫滑**（最多 3 則、固定卡尺寸；點擊開連結並 **`recordAdClickAction`**）） |
...
- **Layer 5**：**`HomeParticlesBackground.tsx`** — **`initParticlesEngine`**（**slim + emitters + image**）；**`Particles`** **`className`**：**`fixed inset-0 z-0 h-[100dvh] pointer-events-none`**（疊於 **`AppShellMotion`** 紫黑漸層之上、**Navbar `z-40`** 與首頁內容 **`z-10`** 之下）。
