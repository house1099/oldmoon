...
- 卡片／Modal：`UserCard.tsx`；**`title-badge-row.tsx`**（稱號胸章＋膠囊；**`sm`／`md`／`lg`／`xl`／`card`**；探索 **`UserCard`** 稱號在**底列**興趣／技能右側 **`card`**（約 **sm×1.1**））；`public/items/title-luffy.png`（稱號胸章範例 **`LUFFY`**，見 **`public/items/README.md`**）；`CardDecorationWrapper.tsx`；`card-decoration.ts`（`CardDecorationConfig`／metadata 解析）；**`modals/UserDetailModal.tsx`**（**`publicPreview`**：不載入社交狀態、隱藏聊聊／緣分／血盟／**master** 信譽與領袖工具）；`LevelBadge`／`LevelCardEffect`；`ShopCardFrameOverlay.tsx`；卡框比例 **`CARD_FRAME_OVERLAY_PERCENT`**（`shop-card-frame-preview.ts`，與頭像框 **`MASTER_AVATAR_FRAME_OVERLAY_PERCENT`** 分離）
...
1. **2026-03-31 — 首頁他人視角預覽 + 商城管理分頁／類型篩選**：**`UserDetailModal`** 新增 **`publicPreview`**（略過 **`getModalSocialStatusAction`**、隱藏聊聊／緣分／血盟／信譽／領袖工具、頂部說明文案）；**`guild-profile-home.tsx`** 大頭貼右下角 **Lucide `Eye`** 開預覽（**`stopPropagation`**，與快捷列同風格圓鈕）；**`shop-admin-client.tsx`** **上架中／已下架** 分頁（**`is_active`**）+ **商品類型** 下拉（**`ITEM_TYPE_OPTIONS`**／**`item_type`**，桌機表與手機卡片共用 **`filteredItems`**）。**`npm run build`** 通過。詳見 **`HANDOFF_HISTORY.md`**（**「首頁他人視角預覽與商城管理分頁／類型篩選（2026-03-31）」**）。
2. **2026-03-31 — 獎池權重輸入與私訊日期**：**`prizes-client.tsx`** 權改用 **`weightStr`** 控制輸入（可清空再輸入）；**`saveAllWeights`／新增獎項** 送出前驗證 **≥1**；機率加總用 **`resolvedPrizeItemWeight`**（空或非法暫回伺服器權重）。**`ChatModal.tsx`** 依 **台北曆日** 插入 **今天／昨天／月日（週）** 分隔列。詳見 **`HANDOFF_HISTORY.md`**（**「獎池權重輸入與私訊日期（2026-03-31）」**）。
3. **2026-03-31 — 贈禮單則通知（一動作一信件／Push）**：**`gift.action.ts`** 匯出 **`formatGiftBatchMailboxMessage`**，**`confirmGiftsToUserBatchAction`** 沿用；**`FloatingToolbar`** 背包贈一般玩家改 **`giftPlayerRewardIds`＋`confirmGiftsToUserBatchAction`**（**`stackMenuQty`** 多件同次送出僅一則通知）；**`rewards.action.ts`** **`giftUserRewardsToAlliancePartnerBatchAction`** 成功後 **`notifyUserMailboxSilent`** 一則摘要（同文案 helper）。商城 **`confirmGiftsToUserBatchAction`** 行為不變。詳見 **`HANDOFF_HISTORY.md`**（**「贈禮單則通知與共用信件文案（2026-03-31）」**）。
4. **2026-03-31 — 酒館 `@` 提及改輸入觸發＋篩選**：**`tavern-mentions.ts`** **`getTavernInlineMentionState`**；**`TavernModal`** 游標同步、直向名單、**`includes`** 篩選、**`↑`／`↓`／`Enter`／`Esc`**、左側 **`@`** 改 **插入 `@`**；與貼圖列互斥。詳見 **`HANDOFF_HISTORY.md`**（**「酒館 @ 提及輸入觸發與篩選（2026-03-31）」**）。
5. **2026-03-31 — 探索 `UserCard` 底列稱號 `card`（約 sm×1.1）**：**`TitleBadgeRow`** 新增 **`size="card"`**（胸章 **1.1rem**、字 **11px**、較窄 **`max-w`**）；**`UserCard`** 底列 **`xl` → `card`**，避免擠壓興趣／技能標籤。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 底列稱號縮小（`card` ≈ sm×1.1，2026-03-31）」**）。
