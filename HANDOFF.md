...
1. **2026-03-31 — 探索 `UserCard` 底列稱號 `card`（約 sm×1.1）**：**`TitleBadgeRow`** 新增 **`size="card"`**（胸章 **1.1rem**、字 **11px**、較窄 **`max-w`**）；**`UserCard`** 底列 **`xl` → `card`**，避免擠壓興趣／技能標籤。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 底列稱號縮小（`card` ≈ sm×1.1，2026-03-31）」**）。
2. **2026-03-31 — 探索 `UserCard` 稱號底列＋LUFFY 胸章資產**：稱號改置 **興趣／技能列右側**（**`items-center`** 垂直對齊）；**`public/items/title-luffy.png`**（**`source-luffy-72.png`** flood-fill 去背＋**1.3×**）、**`scripts/process-title-luffy-png.py`**；**`public/items/README.md`** 後台 **`/items/title-luffy.png`**、名稱 **LUFFY**。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號底列與 LUFFY 胸章（2026-03-31）」**）。
3. **2026-03-31 — 探索 `UserCard` 稱號約 1.3× 與排版**：**`TitleBadgeRow`** 新增 **`size="lg"`**（胸章 **1.3rem**、字 **13px** 等）；**`UserCard`** 曾用頂緣稱號（後已由底列取代）。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號放大與排版（Apple-like，2026-03-31）」**）。
4. **2026-03-31 — 探索列表稱號補齊與卡片頂標**：**`ExploreClient`** 村莊 **`useSWR`** 改 **`revalidateOnMount`／`revalidateIfStale`／`keepPreviousData`**；**`village.service`** **`unstable_cache`** 鍵 **`village-v7`**；**`UserCard`** 稱號曾改 **absolute** 貼 **卡片上緣中央**（避免 flex 擠壓）。詳見 **`HANDOFF_HISTORY.md`**（**「探索列表稱號遺漏：村莊 SWR 重拉、`village-v7`、卡片頂緣標籤」**）。
5. **2026-03-31 — 稱號胸章全站＋裝備同步＋後台**：L2 **`findEquippedTitlesByUserIds`**、**`findEquippedRewardLabels.equippedTitleImageUrl`**；L3 村莊／市集／私訊／酒館／血盟 **`equippedTitle`／`equippedTitleImageUrl`**；L5 **`TitleBadgeRow`**（**`UserCard`／`UserDetailModal`／`guild-profile-home`／公會聊天與血盟列表／`ChatModal`／`TavernModal`**）、**`ExploreClient`** 預載胸章圖；**`FloatingToolbar`** 裝卸 **`title`／`avatar_frame`／`card_frame`** 後 **`guild-rewards-invalidate`** → 首頁 **`loadRewards`**；後台 **`shop-admin`** **`title` optgroup**、**`prizes`** **`title`** 與 **`LocalFrameImagePicker`**（**`items`**）；**`createPrizeItemAction`／`updatePrizeItemAction`** 允許 **`title`** 寫入 **`image_url`／`effect_key`**。詳見 **`HANDOFF_HISTORY.md`**（**「稱號胸章全站顯示與後台（2026-03-31）」**）。
