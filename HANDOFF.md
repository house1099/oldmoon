...
1. **2026-03-31 — 探索 `UserCard` 稱號底列＋LUFFY 胸章資產**：稱號改置 **興趣／技能列右側**（**`items-center`** 垂直對齊標籤）；**`TitleBadgeRow`** 新增 **`size="card"`**（底列約 **sm×1.1**；另保留 **`xl`**）；**`public/items/title-luffy.png`**（白底原圖 **`source-luffy-72.png`** 邊緣 flood-fill 去背＋**1.3×**）、**`scripts/process-title-luffy-png.py`**；**`public/items/README.md`** 後台填 **`/items/title-luffy.png`**、名稱 **LUFFY**。詳見 **`HANDOFF_HISTORY.md`**（**「探索 UserCard 稱號底列與 LUFFY 胸章（2026-03-31）」**）。
