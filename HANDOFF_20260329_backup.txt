...
- **Layer 2（資料）**：`user.repository.ts`（含 **`updateLastCheckinAt`**）、`exp.repository.ts` 支援 **`total_exp`**（SSOT）；**`streak.repository.ts`**（**`login_streaks`**）、**`streak-rewards.repository.ts`**（**`streak_reward_settings`** 七日獎勵設定 CRUD／查詢）、**`prize.repository.ts`**（**`prize_pools`／`prize_items`／`prize_logs`／`user_rewards`**，含獎池／獎項 CRUD 與 log 計數）、**`rewards.repository.ts`**（**`user_rewards` 讀寫裝備**、**`broadcasts`**）；**`chat.repository.ts`**（**`conversations`**／**`chat_messages`**／**`blocks`**／**`reports`**，admin，供私訊／封鎖／檢舉接線）。
- **Layer 3（業務）**：`daily-checkin.action.ts` 之 **`claimDailyCheckin`**（**`login_streaks`** 連簽；**EXP／幣**優先讀 **`streak_reward_settings`**，缺表或缺列時 **fallback** 舊預設；**`coins_max`** 時 **隨機幣**；**`special_reward === 'loot_box'`** 或 fallback 觸發 **`drawFromPool('loot_box')`**）以 **`users.last_checkin_at`** 為簽到 **24h 滾動冷卻** SSOT；**`getStreakRewardSettingsAction`**（**`unstable_cache` 300s** **`tags: ['streak_rewards']`**）；**`exp_logs.unique_key`** 為 **`daily_checkin:{userId}:{timestamp}`**；**`prize-engine.ts`** 通用抽獎 helper；**`rewards.action.ts`**（**`getMyRewardsAction`** 含 **`cardFrames`／`inventorySlots`／`allRewards`**、廣播券、**`getActiveBroadcastsAction`** 快取）；**`admin.action`**：**`getStreakRewardSettingsAdminAction`／`updateStreakRewardAction`**（**master**、**`revalidateTag('streak_rewards')`**）；**`chat.action.ts`**（開啟對話／訊息／列表／封鎖／檢舉）、**`notification.action.ts`**（**`getMyNotificationsAction`**（**直接** **`loadNotificationsForUser`**，**不**包 **`unstable_cache`**；批量載入發送者）／**`insertMailboxNotificationAction`／`notifyUserMailboxSilent`**／**`markNotificationReadAction`**／**`markAllNotificationsReadAction`**／清除／未讀數）。
...
| 頁面切換「開門」過場 | **`src/components/layout/app-shell-motion.tsx`**：**`pathname` 變更**（**`/` 首頁不播**）→ 上／下扇 **`fixed`** 覆蓋**整個視口**（各 **`h-1/2`**、**`z-[9999]`**），**不受內容區高度／`overflow` 裁切**；**`splash`** **`backgroundSize: 100% 200%`**、**`center top`／`bottom`**（**X** 中線接縫）。**時序**：關 **100ms** → 停 **1s** → 開 **1s**。扇門 **`pointer-events-none`**；過場中外層暫 **`pointer-events-auto`** 阻擋誤觸；**idle** 時上下扇分別 **`-translate-y-full`／`translate-y-full`** 完全離開可視區。內容區 **`min-h-[100dvh]`**、**`pt-[calc(2rem+env(safe-area-inset-top,0px))]`** 預留頂部 **`TavernMarquee`**；**無 `overflow-hidden`**；**pb** 預留底欄＋底部 **`bg-zinc-950`** 條避免切頁藍線。**`GuildTabProvider`**／**`FloatingToolbarProvider`** 內 **`Navbar`**（**`z-40`**）與 **`FloatingToolbar`**（主鈕 **z-50**）掛在 **`src/app/(app)/layout.tsx`**，與 **`AppShellMotion`** 同層，**不受開門動畫裁切**；過場層在上，播完 idle 後不擋導航。 |
| 酒館廣場（全頁） | **`TavernMarquee`**（**`src/components/tavern/TavernMarquee.tsx`**）：頂欄跑馬燈最新 5 則、**`useTavern`**；**`(app)/layout`** 傳 **`messageMaxLength`**（**`getMessageLimitsAction().tavernMax`**）給 **`FloatingToolbar` → `TavernModal`**（**`maxLength`**）；全螢幕聊天、貼圖、**master/moderator** 長按刪除與禁言；**`globals.css`** **`.animate-marquee`**。 |
