...
export default function InterestsPage() {
  ...
  async function handleNext() {
    ...
    setSaving(true);
    try {
      const result = await updateMyProfile({ interests: selected });
      if (result.ok === false) {
        toast.error(result.error);
        return;
      }
      router.push("/register/skills-offer");
      router.refresh();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="min-h-screen space-y-6 bg-zinc-950 px-4 py-8">
      <div className="space-y-1">
        <p className="text-xs text-zinc-500">步驟 3 / 5</p>
        <h1 className="text-xl font-bold text-white">你喜歡什麼？</h1>
        <p className="text-sm text-zinc-400">選擇你的興趣，找到同路人</p>
      </div>
