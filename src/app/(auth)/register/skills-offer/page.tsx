...
export default function SkillsOfferPage() {
  ...
  async function handleNext() {
    ...
    setSaving(true);
    try {
      const result = await updateMyProfile({ skills_offer: selected });
      if (result.ok === false) {
        toast.error(result.error);
        return;
      }
      router.push("/register/skills-want");
      router.refresh();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="min-h-screen space-y-6 bg-zinc-950 px-4 py-8">
      <div className="space-y-1">
        <p className="text-xs text-zinc-500">步驟 4 / 5</p>
        <h1 className="text-xl font-bold text-white">你能教什麼？</h1>
        <p className="text-sm text-zinc-400">選擇你能提供的技能</p>
      </div>
